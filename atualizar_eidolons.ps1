...
function Build-PageHtml {
    ...
    [void]$sb.AppendLine("    :root { --bg:#fff6ef; --card:#fffdf9; --ink:#2d1f1a; --muted:#6c5a53; --line:#efddd3; --accent:#cc5f2e; }")
    [void]$sb.AppendLine("    * { box-sizing:border-box; }")
    [void]$sb.AppendLine("    body { margin:0; font-family: `"Segoe UI`", Tahoma, sans-serif; background: radial-gradient(circle at top, #fffdf8, var(--bg)); color:var(--ink); }")
    ...
    [void]$sb.AppendLine("    .topbar { position: sticky; top:0; z-index:10; padding:12px 0; backdrop-filter: blur(6px); background: color-mix(in srgb, #fffaf6 85%, transparent); border-bottom:1px solid var(--line); margin-bottom:18px; }")
    [void]$sb.AppendLine("    .search { width:100%; border:1px solid var(--line); border-radius:10px; padding:10px 12px; font-size:0.95rem; }")
    [void]$sb.AppendLine("    .section { background:var(--card); border:1px solid var(--line); border-radius:12px; padding:12px; margin-top:14px; box-shadow:0 3px 10px rgba(0,0,0,.03); }")
    ...
    [void]$sb.AppendLine("    th { background:#fff4ec; }")
    ...
    [void]$sb.AppendLine("    .combo-item { display:inline-flex; align-items:center; gap:6px; background:#fff7f1; border:1px solid #f0dfd5; border-radius:999px; padding:3px 8px 3px 4px; }")
    [void]$sb.AppendLine("    .eido-icon { width:24px; height:24px; border-radius:50%; object-fit:cover; border:1px solid #e8d3c7; background:#fff; }")
    ...
    [void]$sb.AppendLine("    .combo-sep { color:#9c8174; font-weight:700; }")
    [void]$sb.AppendLine("  </style>")
    [void]$sb.AppendLine("</head>")
    ...
    [void]$sb.AppendLine("    <input id=`"q`" class=`"search`" placeholder=`"Search by Eidolon, combo or bonus...`">")
